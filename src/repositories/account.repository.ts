...
export class AccountRepository extends Repository {
  ...
  public async loginfb({fbAccessToken,dryrun=true,username=null, waterfallId = null, newToken = true}:{fbAccessToken: string,dryrun: boolean,username: string, waterfallId: string, newToken: boolean}): Promise<AccountRepositoryLoginResponseLogged_in_user> {
    const response = await Bluebird.try(() => this.client.request.send<AccountRepositoryLoginResponseRootObject>({
      method: 'POST',
      url: '/api/v1/fb/facebook_signup/',
      form: this.client.request.sign({
          dryrun: dryrun.toString().toLocaleLowerCase(),
          phone_id: this.client.state.phoneId,
          _csrftoken: this.client.state.cookieCsrfToken,
          adid: '',
          guid: this.client.state.uuid,
